...
function removeThings(array, item){
		var res = [];
...
function tripleTheFun(para){
		if(isNaN(para)){return	para + para + para;}
		else {return para * 3;}
...
function clockIn(param1, param2){
	var res = {name: param1,
		department: param2,
		 timestamp:new Date()
	 };
...
// #14 Create a function called spiderTest that takes in an object that is
 // an actor, it looks to see if the actor's name is Andrew Garfield and changes
 // the actor object to have a property called spiderRating with a value of 10.
 // If their name is Tobey Maguire set spiderRating to 0.  otherwise set spiderRating
 // to 'not a spiderman'.
...
/*
    temperature > 85 && humidity > 45 - return "I'm all sweat"
    temperature < 40 && cloudiness > 60 - return "I have icicles"
    temperature > 85 && humidity < 45 && cloudiness < 20 - return "I'm literally in the desert"
    temperature > 90 or humidity > 55 or cloudiness > 65 - return "Hmm, probably not"
...
// && a string parameter(back) and invokes it(holla) with the string(back) + ' back'.
